...
public class JdbcMasterRefreshLogRepository implements MasterRefreshLogRepository {
    private static final List<String> KNOWN_MASTERS = List.of("city", "currency", "transport_mode");
    ...
    public void record(MasterRefreshOutcome outcome, Instant startedAt) {
        jdbcTemplate.update("""
            INSERT INTO travel.master_refresh_log (master_name, status, records_upserted, error_message, started_at, completed_at)
            VALUES (?, ?, ?, ?, ?, now())
    ...
    private MasterStatus statusFor(String masterName) {
        ...
        var latest = jdbcTemplate.query("""
            SELECT status, completed_at FROM travel.master_refresh_log
    ...
    private long countFor(String masterName) {
        var table = switch (masterName) {
            case "city" -> "travel.city";
            case "currency" -> "travel.currency";
            case "transport_mode" -> "travel.transport_mode";
