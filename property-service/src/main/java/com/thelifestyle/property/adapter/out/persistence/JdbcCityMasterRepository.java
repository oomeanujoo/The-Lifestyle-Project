...
public class JdbcCityMasterRepository implements CityMasterRepository {
    ...
    public void upsert(String name, String countryCode, Double latitude, Double longitude) {
        ...
        jdbcTemplate.update("""
            INSERT INTO property.city (name, country_code, latitude, longitude, updated_at)
            VALUES (?, ?, ?, ?, now())
    ...
    @Override
    public long count() {
        Long result = jdbcTemplate.queryForObject("SELECT count(*) FROM property.city", Long.class);
